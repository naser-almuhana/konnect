...
export function useCreatePostMutation() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (newPost) => {
      // Define query filters for the "for-you"  feeds
      const queryFilter = {
        queryKey: ["post-feed", "for-you"],
      } satisfies QueryFilters

      // Cancel any ongoing queries related to the "for-you" feed
      await queryClient.cancelQueries(queryFilter)

      // Update the "for-you" feed cache with the new post
        ...
        (oldData) => {
          ...
          if (firstPage) {
            return {
              ...
              pages: [
                {
                  posts: [newPost, ...firstPage.posts], // Prepend new post to the "for-you" feed
